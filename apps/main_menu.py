...
class main:
    ...
    def load():
        ...
        for i in commands.cmd_list:
            if cmd_counter < y_max:
                cmd_counter += 1
                mvaddstr((cmd_counter + 2), (round(x_max * 0.75) - len(i) // 2), i)
    ...
    def loop():
        ...
        while True:
            ...
            if len(io_stream) == 1:
                ...
                if io_command.startswith(
                    "load"
                ):  # extra if for load() as y'need the name
...
class commands:
    ...
    cmd_list = ["load", "cfg", "quit"]

    user_alias = {"load": load, "cfg": config}  # user typed strs for ran commands
